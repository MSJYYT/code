...
#		print(self.X);print('\n');print(self.labelMat)
	...
	def initparam(self) :
		...
		self.m = shape(self.X)[0]                                                       #数据集行数  200
...
#		print(self.lambdas)
	...
	def train(self) :
		...
		lambdaPairsChanged += self.innerLoop(0)
		'''
		while (step < self.maxIter) and ((lambdaPairsChanged > 0) or (entireflag)) :
			...
			elif (lalambdaPairsChanged == 0) :entireflag = True
	...
	def innerLoop(self, i) :
		...
		if ((self.labelMat[i] * Ei < -self.tol) and (self.lambdas[i] < self.C)) or ((self.labelMat[i] * Ei > self.tol) and (self.lambdas[i] > 0)) :
			...
			self.eCache[i] = [1, self.calcEk[i]]                                                              #计算个更新 j 的缓存
